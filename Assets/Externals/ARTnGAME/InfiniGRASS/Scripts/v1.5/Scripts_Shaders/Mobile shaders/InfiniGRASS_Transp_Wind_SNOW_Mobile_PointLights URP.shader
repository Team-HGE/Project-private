...
Shader "InfiniGRASS/InfiniGrass Directional Wind SNOW Mobile Point Lights URP" {
    Properties {
...
//                    #endif
//                #else
                    float3 lightDirection = normalize(_WorldSpaceLightPosA.xyz);
               // #endif
                
                
                lightDirection =  (reflect(_WorldSpaceLightPosA.xyz,normalDirection));
...
////// Lighting:
               ...
               float3 attenColor = 1 * (_LightColorA.xyz);
...
//                #endif
         //       specular *= ((attenuation) * _LightColor0.xyz);
             ...
             //   finalColor += diffuseLight * (lerp(float3(node_331,node_331,node_331),float3(0.9632353,0.8224623,0.03541304),i.vertexColor.b)*node_1.rgb);
                finalColor += diffuseLight * (node_1.rgb) * _ColorGlobal; //v1.9.6

       //v1.7
       // specular = specular * (i.uv0.y*1-0.5) ;
       //float3 specular = attenuation * (i.uv0.y*1-0.5) ;
       float3 specular =attenuation * (i.uv0.y)*dot(halfDirection,normalDirection) ;
...
//				if(i.uv0.y >= 1-(3 * _SnowCoverage) * col.r* col.r)
//	            {     	       
...
//	                col.rgb = float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*1;                             
//	            }
//	            else
//	            {
//					//col = col * input.color * input.color.a *_UnityTerrainTreeTintColorSM *1.5;						
...
//				}
                ...
                //END SNOW
                float4 col =fixed4(finalColor * _Color  + specular* _SpecularPower * _LightColorA.xyz,1) * 2;//v1.7.7
...
/////// Normals:
                ...
                float3 lightDirection = normalize(lerp(_WorldSpaceLightPosA.xyz, _WorldSpaceLightPosA.xyz - i.posWorld.xyz,_WorldSpaceLightPosA.w));
                float3 halfDirection = normalize(viewDirection+lightDirection);
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
      ...
      //          float3 attenColor = attenuation * (_LightColor0.xyz*(node_133+1)  + _LightColor0.xyz*dot(viewDirection,lightDirection)/1);
                ...
                float3 attenColor = attenuation * _LightColorA.xyz ;
/////// Diffuse:
                float NdotL = dot( normalDirection, lightDirection );
                float3 w = float3(0.9,0.9,0.8)*0.5; // Light wrapping
                float3 NdotLWrap = NdotL * ( 1.0 - w );
...
////// Specular:
				...
				finalColor += diffuseLight * (node_1.rgb) * _ColorGlobal * 5 * attenuation; //v1.9.6

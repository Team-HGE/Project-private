...
Shader "InfiniGRASS/InfiniGrass Directional Wind ROOF URP1 BILLBOARDS"
{
	Properties
	{
	...
	_BaseLight("Grass Base light control", Float) = 0
		...
		_Brightness("Brightness and contrast", Vector) = (1, 1, 1, 0)

		//v2.1.7
		_BillboardWidth("_Billboard Width", Float) = 1
		_BillboardHeight("_Billboard Height Offset", Float) = 0
	}
...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
	...
	float3 ObjSpaceViewDirA(in float4 v)
	{
	...
	Varyings LitPassVertex(Attributes input)
	{
		...
		float4 localVert = input.positionOS;
		//BILLBOARD - v2.1.1 - v2.1.7
		// float3 center = v.vertex;//float3(0,0,0);
		float3 center = input.vertexColor;//float3(0,0,0);
		//float3 center = float3(v.vertex.x,0, v.vertex.z);
		float3 direction = ObjSpaceViewDirA(localVert);
		float3 up = float3(0, 1, 0);
		float3 right = normalize(cross(direction, up))*_BillboardWidth;
		float3 position = float3(center.x, localVert.y, center.z); //v2.1.7 //center
		position += (input.texcoord.x - 0.5) * right;
		position += (input.texcoord.y - 0.5) * up;
		input.normalOS = direction;
		localVert = float4(position, 1);
		if (output.uv.y > 0.1) {
			localVert.y = localVert.y + _BillboardHeight;
		}
		//END BILLBOARDING


		/////////////// INFINIGRASS 1
		VertexPositionInputs vertexInputPre = GetVertexPositionInputs(localVert.xyz);
	...
	//	//END INFINIGRASS 1
		...
		return pow(float4(color.r, color.g, color.b, 0), _Brightness.y) * _Brightness.x;
		}
	#endif			
			ENDHLSL
		}
	...
	#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"
		...
		Varyings DepthOnlyVertex(Attributes input)
		{
			...
			output.uv = TRANSFORM_TEX(input.texcoord, _BaseMap);
			float4 localVert = input.position;
			//BILLBOARD - v2.1.1 - v2.1.7
			// float3 center = v.vertex;//float3(0,0,0);
			float3 center = input.vertexColor.xyz;//float3(0,0,0);
			//float3 center = float3(v.vertex.x,0, v.vertex.z);
			float3 direction = ObjSpaceViewDirA(localVert);
			float3 up = float3(0, 1, 0);
			float3 right = normalize(cross(direction, up))*_BillboardWidth;

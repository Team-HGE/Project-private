...
Shader "InfiniGRASS/InfiniGrass Directional Wind ROOF URP2 SHAPING"
{
	Properties
	{
...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
	...
	Varyings LitPassVertex(Attributes input)
	{
		...
		float3 Origin = float3(_InteractTexturePos.x, _InteractTexturePos.z, 0);
		...
		float2 shapeUVs = input.texcoord;
		...
		input.texcoord.x = input.texcoord.x / tilesX;
		input.texcoord.y = input.texcoord.y / tilesY;
		...
		input.texcoord.x = input.texcoord.x + (ypoloipo - 1)*(1 / tilesX);
		input.texcoord.y = input.texcoord.y + (division + sub)*(1 / tilesY);
